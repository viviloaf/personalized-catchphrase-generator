...
# Prep for the twitter bot
inverse_vocab = gru.inverse_vocab

st.subheader('Bot Stuff')
# Twitter Bot instance
if st.button('Run Bot Instance'):
    while True:
        with st.empty():
            # generate something random in gpt-2
            tweet = ai.twitter_respond(
                passing_input=np.random.choice(inverse_vocab), 
                temperature=np.random.choice(np.arange(0.1,5.0,0.1)))
            tweeter.send_mock_tweet(tweet)
            st.write(tweet)
